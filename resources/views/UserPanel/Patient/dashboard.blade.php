...
<a href="/shareProfile" onclick="getDoctorslist(event)"class="btn btn-light px-4 py-2 text-primary shadow">Share Now</a>
...
<a onclick="Request()" class="btn btn-light px-4 py-2 text-primary shadow" data-bs-toggle="modal" data-bs-target="#submitRequest">Submit Request</a>
</div>
</div>
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
          <form>
            ...
            <button type="button" onclick="submitRequest()" class="btn btn-primary">Save</button>
          </form>
        </div>
    </div>
    </div>
...
<div class="modal fade" id="invitation_detail_modal" tabindex="-1" aria-labelledby="reason-modelLabel" aria-hidden="true">
    <div class="modal-dialog">
    <div class="modal-content">
        ...
        <div class="modal-body">
          <form>
            ...
            <button class="btn btn-light me-2 accept_invitation_btn" style="border-color: #02b2b0;border-width: 2px;" data-id="" onclick="accept_invitation_fun(this)"><img src="/assets/images/check.svg"></button>
            <button class="btn btn-light ml-1 reject_invitation_btn" style="border-color: #02b2b0;border-width: 2px;" data-id="" onclick="reject_invitation(this)"  ><img src="/assets/images/x.svg"></button>
...
<div class="row mt-3 acceptedPatients_upper_container doctor__box d-none">
                        ...
                        <div class="border border-2 border-primary p-2 rounded-3 ">
                            <div class="h5 text-primary d-flex "><strong>New Invites</strong> 
                                <div  class="ml-5  d-flex justify-content-around" style="margin-left: 20px;"> 
                                    ...
                                    <button type="button" class="btn btn-primary btn-sm ml-1 fw-btn btn1 btn-active" style="margin-left: 10px;" onclick="get_patient_invites(0,0)" >New</button> 
                                    <button type="button" class="btn btn-primary btn-sm ml-1 fw-btn btn2 " style="margin-left: 10px;" onclick="get_patient_invites_reject_by_doctor(0,0)">Reject by doctor</button> 
                                    <button type="button" class="btn btn-primary btn-sm ml-1 fw-btn btn3 " style="margin-left: 10px;" onclick="get_patient_invites_reject_by_patient(0,0)">Reject by patient</button> 
                                </div> 
         ...
         <div class="col-md-6 widget-card-col  ">
               ...
               <div  class="ml-auto mr-auto d-flex justify-content-center pages_btn2 mt-2" style="margin-left:auto !important;margin-right:auto !important;"> 
...
function fetch_all_modules_data(){
    ...
    if (width >= 425) {
        ...
        function details(){
            var $html= `<div id="moduleManagersList" class="h5"><strong>👋 Welcome Back - <small class="text-primary">${getEmail('user_email')}<small></small></strong></div>`;
            $('.user_email').html($html);
        }


        function nameRole(){
                var $html= `<div  class="h3 class="p-0 m-0 bread-crumb-heading"><strong><small>  Dashboard -</small>${getName('nameRole')}</strong></div>`;
                $('.nameRole').html($html);
            }
            function Title() {
    var nameRole = getName('nameRole');
    document.title = nameRole + " Dashboard - {{env('APP_NAME')}}";
    var $html = `<div class="h3 p-0 m-0 bread-crumb-heading"><strong>${nameRole}<small>  Dashboard -</small></strong></div>`;
    $('.title').html($html);
}
...
function invitation_detail(_this) {
    ...
    $('.PatientNoteDiv').html($(_this).attr('data-patient-note') != null &&  $(_this).attr('data-patient-note') != ''  ? 'Patient Note:<br/> '+$(_this).attr('data-patient-note') : '' );
    $('.DoctorNoteText').val('');
...
function get_patient_invites(from,pagination){
   ...
   success: function(response) {
    ...
    {
        pages_btn += `<button type="button" class="btn btn-primary btn-sm ml-1  ${response.data.count == 0 ? 'invisible' : '' }" style="margin-left:10px;" onclick="get_patient_invites(${c*10},1);" >${c+1}</button>`;
    }
...
detail.forEach(function(request) {
        ...
        html += '<button class="me-2" data-id="' + request.id + '" data-patient-id="' + request.patient_id + '" data-patient-note="' + request.patient_note + '" data-hide-reject-btn="0" data-hide-accept-btn="0" onclick="invitation_detail(this);"><i class="fas fa-tasks" style="font-size: 20px;padding: 5px;color:#02b2b0 !important;" ></i></button>';
...
function get_patient_invites_reject_by_patient(from,pagination){
   ...
   success: function(response) {
    ...
    for(var c = 0; c < count; c++)
    {
        pages_btn += `<button type="button" class="btn btn-primary btn-sm ml-1  ${response.data.count == 0 ? 'invisible' : '' }" style="margin-left:10px;" onclick="get_patient_invites_reject_by_patient(${c*10},1);" >${c+1}</button>`;
    }
...
function get_patient_invites_reject_by_doctor(from,pagination){
   ...
   success: function(response) {
    ...
    for(var c = 0; c < count; c++)
    {
        pages_btn += `<button type="button" class="btn btn-primary btn-sm ml-1  ${response.data.count == 0 ? 'invisible' : '' }" style="margin-left:10px;" onclick="get_patient_invites_reject_by_doctor(${c*10},1);" >${c+1}</button>`;
    }
...
var html2 = '';
  ...
  var pages_btn = ``;
    ...
    for(var c = 0; c < count; c++)
    {
        pages_btn += `<button type="button" class="btn btn-primary btn-sm ml-1  ${response.data.count == 0 ? 'invisible' : '' }" style="margin-left:10px;" onclick="get_my_patients(${c*10},'',1);" >${c+1}</button>`;
    }
...
detail.forEach(function(request) {
        ...
        html2 += '<div class="patient-id" style="font-weight: 400;" >' + '<b>Patient ID:</b> ' + request.patient_id + '</div>';
        html2 += '<div class="patient-message"  >' + '<b>Accepted Date:</b> ' + request.accepted_date + '</div>';
        html2 += '</div>';
        html2 += '<div class="action-section">';
        html2 += '<button class="me-2" data-id="' + request.id + '" data-patient-id="' + request.patient_id + '"   data-patient-note="' + request.patient_note + '" data-hide-reject-btn="0" data-hide-accept-btn="1"  onclick="invitation_detail(this);"><i class="fas fa-tasks" style="font-size: 20px;padding: 5px;color:#02b2b0 !important;" ></i></button>';
        html2 += '<button class="view-profile" data-patient-id="' + request.patient_id + '" style="padding-top: 11px;" ><img src="/assets/images/user.svg"></button>';
        html2 += '</div>';
        html2 += '</div>';
        html2 += '</div>';
        html2 += '</div>';
