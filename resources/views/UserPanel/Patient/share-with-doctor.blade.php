        ...
        <!--</div>-->
           <div class="col-6">
        <a class="btn btn-primary" id="showModalButton" href="{{url('/shareProfileForm')}}">Share profile</a>
        </div>
        ...
        <input type="text" class="form-control doctor-search" id="doctor-search" placeholder="Search doctor">
        ...
        function getDoctorslist(from=0) {
        ...
        var request_to_doctor = response.data.request_to_doctor;
        ...
        var dateObj = new Date(request_to_doctor[i].date);
        ...
        <td>${request_to_doctor[i].user_data.email}</td>`;
        
        userMeta = request_to_doctor[i].user_data.userMeta ? request_to_doctor[i].user_data.userMeta : [];
        ...
        <td>${request_to_doctor[i]?.accepted_date ? request_to_doctor[i]?.accepted_date : ""}</td>
        
       
        
        if (${request_to_doctor[i]?.doctor_accept_or_reject === 0 && request_to_doctor[i]?.patient_accept_or_reject === 1}) {
        
        <td>
       ${request_to_doctor[i]?.doctor_accept_or_reject == 0 && request_to_doctor[i]?.patient_accept_or_reject == 1
    ? `<span class="btn btn-warning">Pending</span>
       <a class="btn btn-danger btn-sm decline-button" onclick='$("#modalTitle").text("Rejection Note"); $("#rejectButton").text("Reject"); $("#rejectModal").modal("show")' data-id="${request_to_doctor[i].id}">
           <span class="action-items-status badge text-white p-1" title="Response">X</span>
       </a>`
    : request_to_doctor[i]?.doctor_accept_or_reject == 1 && request_to_doctor[i]?.patient_accept_or_reject == 1
    ? `<span class="btn btn-success">Accepted</span>
       <a class="btn btn-danger btn-sm decline-button" onclick='$("#modalTitle").text("Remove Note"); $("#rejectButton").text("Remove"); $("#rejectModal").modal("show")' data-id="${request_to_doctor[i].id}">
           <span class="action-items-status badge text-white p-1" title="Response">X</span>
       </a>`
    : ""
}
...
if (html.trim() === '') {
    ...
    pages_btn += `<button type="button" class="btn btn-outline-primary btn-sm" onclick="getDoctorslist(${prevPage});">Previous</button>`;
    ...
    for (var c = 0; c < Math.ceil(count / 10); c++) {
        ...
        if (from >= pageStart && from < pageEnd) {
            pages_btn += `<button type="button" class="btn btn-outline-primary btn-sm p-btn active" style="margin-left:10px;" onclick="getDoctorslist(${pageStart});">${c + 1}</button>`;
        } else {
            pages_btn += `<button type="button" class="btn btn-outline-primary btn-sm p-btn" style="margin-left:10px;" onclick="getDoctorslist(${pageStart});">${c + 1}</button>`;
        }
    }

    pages_btn += `<button type="button" class="btn btn-outline-primary btn-sm" style="margin-left:10px;" onclick="getDoctorslist(${nextPage});">Next</button>`;
    ...
    $('.pagination-container').html(pages_btn);
        ...
        url: `{{config('app.api_url')}}/api/users/request-status-update-patient?notification_id=${id}&patient_note=${patientNote}&patient_accept_or_reject=${patient_accept_or_reject}`,
        ...
        setCookie('doctor_id',response.data.request_to_doctor.id, '1')
