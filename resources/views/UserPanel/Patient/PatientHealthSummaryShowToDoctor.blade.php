      ...
      <div class="container-fluid p-3 main-container-content">
         <div class="row w-100 justify-content-between border border-3 border-primary border-top-0 border-bottom-0 border-end-0 ps-2 ms-0 mb-3">
          ...
          <a type="button" href="javascript:void(3);" class="btn btn-primary my-2 ps-3 pe-3 nav-link px-0 show_notification btn btn-primary position-relative">
            ...
            <div class="row d-flex justify-content-center mb-3 ">
                <div class="col-6">
                    <div style="text-align: center;">Select Patient: </div>
                    <select class="form-control mt-1 patient_select" onchange="setCookie('section_patient',this.value, 1); window.location.href='/show-patient-health-summary-to-doctor'; " style="width:100px;margin: auto;">
         ...
         <div class="d-flex  justify-content-center">
              <div class="d-none tab-class tab-class-1 tab-class-active" role="button" onclick="$('.tab-box').addClass('d-none');$('.tab-box-1').removeClass('d-none');  $('.tab-class').removeClass('tab-class-active');$('.tab-class-1').addClass('tab-class-active');">HEALTH SUMMARY</div>
               <div class="d-none tab-class tab-class-2 ml-class" role="button" onclick="$('.tab-box').addClass('d-none');$('.tab-box-2').removeClass('d-none');  $('.tab-class').removeClass('tab-class-active');$('.tab-class-2').addClass('tab-class-active');">GERNAL HISTROY</div>
         </div>
...
get_my_patients_arr();
getPatientHealthSummary(getCookie('section_patient'),3);
                 ...
                 html += `
                        <div class="row box">
                            <div class="box_head d-flex">
                                ...
                                <div><button class="btn btn-secondary btn-sm ml-4" style="font-size:10px;margin-bottom:3px;" onclick="open_section_data_list_page(${_meta[key]['id']},'${_meta[key]['name']}','${_meta[key]['table_name']}');" >Edit</button></div>
...
function get_my_patients_arr(){
  ...
  "Authorization": `Bearer ${getCookie('BearerToken')}`,
    ...
    type: "POST", 
    url: `{{config('app.api_url')}}/api/doctor/my-patient-all`,
    data: {
...
{
    html += `<option value="${response[0][i].patient_id}" ${ response[0][i].patient_id == getCookie('section_patient') ? 'selected' : '' } > ${response[0][i].patient_id}</option>`
}
